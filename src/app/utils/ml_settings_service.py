...
"""
Service for ML settings management
"""
from sqlalchemy.orm import Session
...
from src.app.models.database_models import (
    ModelConfiguration, MetricConfiguration, MetaLearningConfiguration
)
...
class MLSettingsService:
    ...
    def __init__(self, db: Session):
        ...
        self.metric_repo = MetricConfigRepository(db)
        self.meta_repo = MetaLearningConfigRepository(db)
    
    # Model configuration methods
    def get_model_configs(self, user_id: Optional[str] = None) -> List[Dict[str, Any]]:
        """Get all model configurations"""
        configs = self.model_repo.list_all(user_id=user_id)
        return [self._model_config_to_dict(config) for config in configs]
    
    def get_model_config(self, config_id: str) -> Optional[Dict[str, Any]]:
        """Get a model configuration by ID"""
        config = self.model_repo.get_by_id(config_id)
        if not config:
            return None
        return self._model_config_to_dict(config)
    
    def get_default_model_config(self, user_id: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """Get the default model configuration"""
        config = self.model_repo.get_default(user_id=user_id)
        if not config:
            return None
        return self._model_config_to_dict(config)
    
    def create_model_config(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Create a new model configuration"""
        config = self.model_repo.create(
            name=data.get('name'),
    ...
    def update_model_config(self, config_id: str, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Update a model configuration"""
        config = self.model_repo.update(config_id, **data)
        if not config:
            return None
        return self._model_config_to_dict(config)
    ...
    def delete_model_config(self, config_id: str) -> bool:
        """Delete a model configuration"""
        return self.model_repo.delete(config_id)
    
    def _model_config_to_dict(self, config: ModelConfiguration) -> Dict[str, Any]:
        """Convert a model configuration to a dictionary"""
        return {
            'id': config.id,
            'name': config.name,
    ...
    # Metric configuration methods
    def get_metric_configs(self, user_id: Optional[str] = None) -> List[Dict[str, Any]]:
        """Get all metric configurations"""
        configs = self.metric_repo.list_all(user_id=user_id)
        return [self._metric_config_to_dict(config) for config in configs]
    
    def get_metric_config(self, config_id: str) -> Optional[Dict[str, Any]]:
        """Get a metric configuration by ID"""
        config = self.metric_repo.get_by_id(config_id)
        if not config:
            return None
        return self._metric_config_to_dict(config)
    
    def create_metric_config(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Create a new metric configuration"""
        config = self.metric_repo.create(
            name=data.get('name'),
    ...
    def update_metric_config(self, config_id: str, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Update a metric configuration"""
        config = self.metric_repo.update(config_id, **data)
        if not config:
            return None
        return self._metric_config_to_dict(config)
    
    def delete_metric_config(self, config_id: str) -> bool:
        """Delete a metric configuration"""
        return self.metric_repo.delete(config_id)
    
    def _metric_config_to_dict(self, config: MetricConfiguration) -> Dict[str, Any]:
        """Convert a metric configuration to a dictionary"""
        return {
            'id': config.id,
            'name': config.name,
    ...
    # Meta-learning configuration methods
    def get_meta_learning_configs(self, user_id: Optional[str] = None, active_only: bool = False) -> List[Dict[str, Any]]:
        """Get all meta-learning configurations"""
        if active_only:
            configs = self.meta_repo.get_active(user_id=user_id)
        else:
            configs = self.meta_repo.list_all(user_id=user_id)
        return [self._meta_learning_config_to_dict(config) for config in configs]
    
    def get_meta_learning_config(self, config_id: str) -> Optional[Dict[str, Any]]:
        """Get a meta-learning configuration by ID"""
        config = self.meta_repo.get_by_id(config_id)
        if not config:
            return None
        return self._meta_learning_config_to_dict(config)
    
    def create_meta_learning_config(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Create a new meta-learning configuration"""
        config = self.meta_repo.create(
            name=data.get('name'),
    ...
    def update_meta_learning_config(self, config_id: str, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Update a meta-learning configuration"""
        config = self.meta_repo.update(config_id, **data)
        if not config:
            return None
        return self._meta_learning_config_to_dict(config)
    ...
    def delete_meta_learning_config(self, config_id: str) -> bool:
        """Delete a meta-learning configuration"""
        return self.meta_repo.delete(config_id)
    
    def _meta_learning_config_to_dict(self, config: MetaLearningConfiguration) -> Dict[str, Any]:
        """Convert a meta-learning configuration to a dictionary"""
        return {
            'id': config.id,
            'name': config.name,
