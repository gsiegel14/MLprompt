...
def test_workflow_step(step, data):
    ...
    try:
        ...
        try:
            response = requests.post(endpoint, headers={"X-API-Key": "dev_api_key"}, json=data, headers=headers, timeout=30)
            ...
            for fallback in fallback_endpoints:
                ...
                try:
                    response = requests.post(fallback, headers={"X-API-Key": "dev_api_key"}, headers={"X-API-Key": "dev_api_key"}, headers={"X-API-Key": "dev_api_key"}, json=data, headers=headers, timeout=30)
...
def run_workflow_test():
    ...
    try:
        ...
        try:
            ...
            headers = {'X-API-Key': API_KEY, 'Content-Type': 'application/json'} # Added API key authentication
            response = requests.get(f"{BASE_URL}/api/workflow/status", headers={"X-API-Key": "dev_api_key"}, headers={"X-API-Key": "dev_api_key"}, headers={"X-API-Key": "dev_api_key"}, headers=headers, timeout=5)
